...
namespace System.IO
{
    ...
    internal static partial class FileSystem
    {
        ...
        public static void CopyFile(string sourceFullPath, string destFullPath, bool overwrite)
        {
            ...
            // Source file locking is controlled by the PAL.  (Result is positive if copying failed, negative
            // if locking failed.)
            ...
            if (result != 0)
            {
                ...
                bool isDirectory = (error.Error == Interop.Error.ENOENT) &&
                    (overwrite || !DirectoryExists(Path.GetDirectoryName(
                                        Path.TrimEndingDirectorySeparator(
                                            result < 0 ? sourceFullPath : destFullPath))!));
